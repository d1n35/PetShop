...
def cadastro_pets():
    ...
    def listar_pets():
        ...
        for row in cursor.fetchall():
            listbox_pets.insert(tk.END, f'ID: {row[0]} | Nome: {row[1]} | Dono: {row[2]} | Dono: {row[3]} | Cuidados e alergias: {row[4]}')
...
def administrar_pacotes():
    ...
    def listar_pacotes():
        ...
        cursor.execute('SELECT id_pet, id_pacote, dataInicio, dataFim FROM pacotes ORDER BY 4')
        for row in cursor.fetchall():
    ...
    def deletar_pacote():
        ...
        if selecao:
            try:
                ...
                id_pet = int(item.split('|')[0].split(':')[1].strip())
                id_pacote = int(item.split('|')[2].split(':')[1].strip())
                cursor.execute(f'DELETE FROM pacotes WHERE id_pet = {id_pet} AND id_pacote = {id_pacote}')
                print(f'DELETE FROM pacotes WHERE id_pet = {id_pet} AND id_pacote = {id_pacote}')
                conn.commit()
...
# Interface gráfica
    ...
    idpet = [(pid[0],"|",pid[1]) for pid in cursor.fetchall()]

    tk.Label(root, text='Pet:').grid(row=0, column=0)
    entry_id = ttk.Combobox(root, textvariable=idpet, values=idpet)
...
def marcar_horarios():
    ...
    def listar_horarios():
        ...
        cursor.execute('SELECT id_pet, data FROM agendamentos ORDER BY 2')
        for row in cursor.fetchall():
